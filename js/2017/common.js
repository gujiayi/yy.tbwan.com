...
**/
	...
	function winSetHP(){
		if(document.all){
			...
			document.body.setHomePage('http://www.505pk.com/'); 
		}else{ 
			popTips('当前浏览器不支持此操作, 请手动设为首页'); 
		}
	}
	// 收藏
	function winAddFav(){
		try{ 
			window.external.addFavorite('http://www.505pk.com/', '505游戏平台'); 
		} catch(e) {  
			try{ 
				window.sidebar.addPanel('505游戏平台', 'http://www.505pk.com/', ''); 
			}catch (e) { 
...
function toDesktop(sUrl,sName){
	...
	function logout(){
		...
		$.post(C9377.app_url+'/login.php', param, function(result) {
	...
	function getUserInfo(){
		$.getJSON(C9377.app_url+'/api/user_info_jsonp.php?level=1&last_game=1&num=6&callback=?', function(json){
			$('#link_user').click(function(){
				...
				window.location.href='/users/users_index.php';
			})
			if(!json.LOGIN_ACCOUNT){
