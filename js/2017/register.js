...
rules: {
	LOGIN_ACCOUNT: {
		...
		remote: {
			url: '/check_user.php',
			cache: false,
			data: {'do': 'user', 'new': 1, username: function(){ return $('#username1').val(); }},
			beforeSend: function(){}
		}
	},
	...
	code: {
		...
		remote: {
			url: '/check_user.php',
			cache: false,
			data: {'do': 'code', 'new': 1, code: function(){ return $('#code1').val(); }},
			beforeSend: function(){}
		}
	},
	// captcha: {
...
var phone = $('#register-form input[name=phone]'), vphone = phone.val(), ptip = phone.closest('.w-item').find('.w-tips'),
	...
	// ydcode = $('#register-form .tncode'), vydcode = ydcode.val(), ytip = ydcode.closest('.w-item').find('.w-tips'),
	// ydcode = $('#register-form .tncode'), vydcode = ydcode.attr('status'), ytip = ydcode.closest('.w-item').find('.w-tips'),
	vbtn = $(btn);
if (vphone.length < 11) {
...
// }
// if (!vydcode.length) {
// 	ytip.html('<label for="ydcode" generated="true" class="error">向右拖动滑块填充拼图</label>')
// 	return false;
// }
...
rules: {
	...
	code: {
		...
		remote: {
			url: '/check_user.php',
			cache: false,
			data: {'do': 'code', 'new': 1, code: function(){ return $('#code2').val(); }},
			beforeSend: function(){}
		}
	},
	TERMS: {
...
var phone = $('#phone'), vphone = phone.val(), ptip = phone.closest('.w-item').find('.w-tips'),
	code = $('#code2'), vcode = code.val(), ctip = code.closest('.w-item').find('.w-tips'), 
	// ydcode = $('#register-phone .'), vydcode = ydcode.val(), ytip = ydcode.closest('.w-item').find('.w-tips'),
			// ydcode = $('#register-phone .tncode'), vydcode = ydcode.attr('status'), ytip = ydcode.closest('.w-item').find('.w-tips'),
			vbtn = $(btn);
if (vphone.length < 11) {
	ptip.html('<label for="phone" generated="true" class="error">请填写正确手机号</label>')
	return false;
...
// 帐号注册
// 	initNECaptcha({
// 		captchaId: 'c6459122009e4ddc981ae4cd9d33b375',
// 		element: '#reg-yd-captcha',
// 		mode: 'float',
// 		width: '320px',
// 		height: '36px',
// 		onVerify: function(err, ret){
// 			if(err){
// 				$('.js-ydcode-reg .w-tips').html('<label class="error">验证码错误</label>')
// 			}
// 			if(ret){
// 				$('.js-ydcode-reg .w-tips').html('<label class="correct">&nbsp;</label>')
// 			}
// 		}
// 	}, function (instance) {
...
// 	initNECaptcha({
// 		captchaId: 'c6459122009e4ddc981ae4cd9d33b375',
// 		element: '#phone-yd-captcha',
// 		mode: 'float',
// 		width: '320px',
// 		height: '36px',
// 		onVerify: function(err, ret){
// 			if(err){
// 				$('.js-ydcode-phone .w-tips').html('<label class="error">验证码错误</label>')
// 			}
// 			if(ret){
// 				$('.js-ydcode-phone .w-tips').html('<label class="correct">&nbsp;</label>')
// 			}
// 		}
// 	}, function (instance) {
// 		// 初始化成功后得到验证实例instance，可以调用实例的方法
